...
export default function SettingsScreen() {
  ...
  async function uploadPhoto() {
    ...
    if (!res.canceled) {
      setUser((p) => ({ ...p, photoUri: res.assets[0].uri }));
    }
  }
      ...
      <View style={s.fieldBlock}>
        ...
        <View style={s.segmentWrap}>
          <Pressable
            ...
            style={[
              s.segmentHalf,
              user.team === "Cats" ? s.segmentActive : s.segmentInactive,
            ]}
          >
            <Text
              style={[
                s.segmentTxt,
                user.team === "Cats" && s.segmentTxtActive,
              ]}
            >
              Cats
            </Text>
          ...
          <Pressable
            ...
            style={[
              s.segmentHalf,
              user.team === "Dogs" ? s.segmentActive : s.segmentInactive,
            ]}
          >
            <Text
              style={[
                s.segmentTxt,
                user.team === "Dogs" && s.segmentTxtActive,
              ]}
            >
              Dogs
            </Text>
        ...
        <View style={{ marginTop: 22 }}>
          ...
          <Pressable
            ...
            onPress={async () => {
              ...
              if (!ok) return;
              await clearAllPosts();
              Alert.alert("Done", "All posts cleared.");
            }}
          >
          ...
          <Pressable
            ...
            onPress={async () => {
              const ok = await confirm(
                "Reset Daily Votes",
                "Clear daily vote history for this device?"
              );
              if (!ok) return;
              const { clearVoteHistoryForAdmin } = await import(
                "../../src/lib/votes"
              );
              await clearVoteHistoryForAdmin();
              Alert.alert("Done", "Daily vote history cleared.");
            }}
          >
          ...
          <Pressable
            ...
            onPress={async () => {
              const ok = await confirm(
                "Clear Winners",
                "Remove ALL saved weekly winners?"
              );
              if (!ok) return;
              const { clearWinners } = await import(
                "../../src/lib/winners"
              );
              await clearWinners();
              Alert.alert("Done", "All winners cleared.");
            }}
          >
...
const s = StyleSheet.create({
  ...
  input: {
    ...
    borderWidth: 1,
    borderColor: "rgba(59,38,26,0.12)",
  },
